...
class Services_WTF_Test
{
    ...
    protected function query($command, $req = 'GET', $params = '')
    {
        ...
        if ($req == 'POST') curl_setopt($ch, CURLOPT_POSTFIELDS, $params);
        
        $results = curl_exec($ch);
        if ($results === false) $this->error = curl_error($ch);
    ...
    public function test($data)
    {
        ...
        if ($result != false)
        {
            ...
            if (empty($result['error']))
            {
                ...
                if (isset($result['state']) and ! empty($result['state'])) $this->result = $result;
    ...
    public function poll_state()
    {
        ...
        {
            if ($this->result['state'] == "completed") return true;
        }
        ...
        if ($result != false)
        {
            ...
            $this->result = $result;
            if ($result['state'] == 'error') $this->error = $result['error'];
    ...
    public function get_results()
    {
        ...
        {
            if ($this->state() == 'completed' or $this->state() == 'error') break;
            sleep($i ++ <= 13 ? 2 : 5);
        }

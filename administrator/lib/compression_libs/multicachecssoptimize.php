...
class MulticacheCSSOptimize extends MulticacheOptimize
{
    ...
    public static function optimize($css, $options = array())
    {
        ...
        $obj = new MulticacheCSSOptimize($options);
    ...
    private function _optimize($css)
    {
        ...
        $css = preg_replace("#(?>(?:[{};]|^)[^{}@;]*+{|(?:(?<![,;:{}])\s++(?![,;:{}]))?[^\s{};\"'(]*+(?:$e|[\"'({};])?)+?\K" . "(?:\s++(?=[,;:{}])|(?<=[,;:{}])\s++|\K$)#s", '', $css);
        ...
        $css = preg_replace("#(?>\(?[^\"'(]*+(?:$s)?)*?(?:(?<=\burl)\(\K\s++|\G" . "(?(?=[\"'])['\"][^'\"]++['\"]|[^\s]++)\K\s++(?=\))|$\K)#s", '', $css);
        
        // minimize hex colors
        $css = preg_replace("/(?>\#?[^\#\"'(]*+(?:$e|\()?)*?(?:(?<!=)\#\K" . "([a-f\d])\g{1}([a-f\d])\g{2}([a-f\d])\g{3}(?=[\s;}])|$\K)/is", '$1$2$3', $css);
